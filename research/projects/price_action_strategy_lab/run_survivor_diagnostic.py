...
from research.projects.price_action_strategy_lab.survivor_diagnostic import SurvivorDiagnosticConfig
from research.projects.price_action_strategy_lab.survivor_diagnostic import run_survivor_diagnostic
from research.projects.price_action_strategy_lab.universe_adapter import to_alpha101_panel


@dataclass(frozen=True)
class SurvivorDiagnosticRunResult:
    report_dir: Path


def run_survivor_diagnostic_config(config_path: str | Path) -> SurvivorDiagnosticRunResult:
    raw = _read_config(Path(config_path))
    panel = to_alpha101_panel(_load_panel(raw))
    result = run_survivor_diagnostic(
        panel,
        default_alpha_registry(),
        default_activator_registry(),
        _config(raw),
    )
    return SurvivorDiagnosticRunResult(result.report_dir)
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_survivor_diagnostic_config(args.config)
    print(f"wrote survivor diagnostic reports to {result.report_dir}")
    return 0


def _config(raw: dict[str, Any]) -> SurvivorDiagnosticConfig:
    ...
    return SurvivorDiagnosticConfig(
        hypothesis=_hypothesis(dict(raw["hypothesis"])),
