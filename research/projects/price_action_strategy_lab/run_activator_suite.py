...
from research.projects.price_action_strategy_lab.activator_suite import ActivatorSuiteConfig
from research.projects.price_action_strategy_lab.activator_suite import run_activator_suite
from research.projects.price_action_strategy_lab.activator_suite import write_activator_suite_reports
...
def run_activator_suite_config(config_path: str | Path) -> ActivatorSuiteRunResult:
    config = _read_config(Path(config_path))
    panel = _load_panel(config)
    result = run_activator_suite(
        to_alpha101_panel(panel),
        default_alpha_registry(),
        default_activator_registry(),
        _suite_config(config),
    )
    paths = write_activator_suite_reports(result, _report_dir(config))
    return ActivatorSuiteRunResult(_report_dir(config), paths[0], paths[1], paths[2], paths[3])
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_activator_suite_config(args.config)
    print(f"wrote activator suite reports to {result.report_dir}")
    return 0
...
def _suite_config(config: dict[str, Any]) -> ActivatorSuiteConfig:
    ...
    return ActivatorSuiteConfig(
        ...
        report_dir=Path(str(compute.get("report_dir", "research/projects/price_action_strategy_lab/reports"))),
        screen_mode=str(screen.get("mode", "ranked_long_only")),
        screen_horizon=int(screen.get("horizon", 10)),
        screen_cost_bps=float(screen.get("cost_bps", 10.0)),
        ...
        min_lift_bps=float(screen.get("min_lift_bps", 0.0)),
