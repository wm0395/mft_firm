...
def show_competition(repository: DataRepository, asset_id: str | None, direction: str | None) -> int:
    rows = []
        ...
        rows.append(
            {
                ...
                "competition": load_json(evaluation.explanation_json).get("competition", {}),
            }
        )
    emit(rows)
    return 0
...
def show_explanation(repository: DataRepository, evaluation_id: str) -> int:
    ...
    if evaluation is None:
        emit({"error": f"Evaluation {evaluation_id} not found"})
        return 1
    emit(load_json(evaluation.explanation_json))
    return 0
...
def show_validation_path(repository: DataRepository, evaluation_id: str) -> int:
    ...
    if evaluation is None:
        emit({"error": f"Evaluation {evaluation_id} not found"})
        return 1
    emit(load_json(evaluation.validation_result_json))
    return 0
...
def report_hypotheses(repository: DataRepository, horizon: int) -> int:
    ...
    emit(
        [
            {"hypothesis_id": hypothesis_id, "horizon": horizon, **aggregate_signal_performance(evaluations, horizon_index).__dict__}
...
def backtest_results(repository: DataRepository) -> int:
    emit([result.__dict__ for result in repository.get_backtest_results()])
    return 0
...
def hypothesis_performance(repository: DataRepository) -> int:
    ...
    emit(
        {
            "trade_outcomes": analyze_hypothesis_performance(repository.get_trade_outcomes()),
            "signal_metrics": [
                {
...
def regime_analysis(repository: DataRepository, asset_symbol: str) -> int:
    ...
    if len(market_data) < 20:
        emit({"error": f"Insufficient data for regime analysis: {len(market_data)} bars"})
        return 1
...
def lineage_trace(repository: DataRepository, signal_type: str | None, hypothesis_id: str | None) -> int:
    rows = []
...
def position_management(
    ...
    emit(
        [
            position.__dict__
            for position in repository.get_positions(asset_id=asset_id, hypothesis_id=hypothesis_id, status=status)
        ]
    )
    return 0


def advanced_report(repository: DataRepository, hypothesis_id: str, asset_id: str | None) -> int:
    emit(
        {
            "hypothesis_id": hypothesis_id,
            "asset_id": asset_id,
            ...
            "backtests": [
                result.__dict__ for result in repository.get_backtest_results() if result.hypothesis_id == hypothesis_id
            ],
            "trade_outcomes": [
                outcome.__dict__ for outcome in repository.get_trade_outcomes() if outcome.hypothesis_id == hypothesis_id
            ],
            "signal_evaluations": [
                evaluation.__dict__
                for evaluation in repository.get_signal_evaluations()
