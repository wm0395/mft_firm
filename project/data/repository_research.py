...
def _rows(statement: str, record_type: Any, db: Any, parameters: tuple[Any, ...] = ()) -> tuple[Any, ...]:
    return tuple(record_type(*row) for row in db.fetch_all(statement, parameters))
...
def _research_artifact_record(
    ...
    if isinstance(artifact, ResearchArtifactRecord):
        return artifact, UPSERT_RESEARCH_ARTIFACT_SQL
...
class RepositoryResearchMixin:
    ...
    def get_research_universes(self) -> tuple[ResearchUniverse, ...]:
        return _load_research_artifacts(
            ...
            "from research_universes order by universe_id",
            ResearchUniverseRecord,
            _db(self),
        )
    ...
    def get_dataset_snapshots(self) -> tuple[DatasetSnapshot, ...]:
        return _load_research_artifacts(
            ...
            "data_end, asset_ids_json from dataset_snapshots "
    ...
    def get_strategy_specs(self) -> tuple[StrategySpec, ...]:
        return _load_research_artifacts(
            ...
            "name, parameters_json from strategy_specs order by strategy_spec_id",
            StrategySpecRecord,
            _db(self),
        )

    def get_strategy_spec(self, hypothesis_id: str, hypothesis_version: int) -> StrategySpec | None:
        for strategy_spec in self.get_strategy_specs():
            if strategy_spec.hypothesis_id == hypothesis_id and strategy_spec.hypothesis_version == hypothesis_version:
                return strategy_spec
        return None

    def get_research_runs(self) -> tuple[ResearchRun, ...]:
        return _load_research_artifacts(
            "select research_run_id, strategy_spec_id, dataset_snapshot_id, started_at, "
            "completed_at, status, notes from research_runs order by started_at, research_run_id",
            ResearchRunRecord,
            _db(self),
        )
    ...
    def get_strategy_evidence_summaries(self) -> tuple[StrategyEvidenceSummary, ...]:
        return _load_research_artifacts(
            "select evidence_summary_id, strategy_spec_id, research_run_id, dataset_snapshot_id, "
            "summary, metrics_json, created_at from strategy_evidence_summaries "
