...
SUPPORTED_ASSET_CLASSES = ("mcx_commodity_future", "mcx_commodity_option")
...
def parse_sample_payload(
    ...
    if asset_class not in SUPPORTED_ASSET_CLASSES:
        raise ValueError(f"unsupported MCX asset class: {asset_class}")
    rows = _selected_rows(payload, asset_class)
    canonical_records = tuple(_canonical_record(row, fixture_path) for row in rows)
    metadata_records = tuple(record for row in rows for record in _metadata_records(row, fixture_path))
    raw_file = build_raw_file_record(
        SOURCE_ID,
        asset_class,
        fixture_path,
        fixture_text,
        _text(payload.get("source_url", "https://www.mcxindia.com")),
        _text(payload.get("sample_date", _timestamp_date(rows[0]))),
        "prototype fixture for MCX official bhavcopy",
    )
    quality_report = build_quality_report(
        ...
        notes="prototype sample only; contract archives remain non-production",
    )
...
def _selected_rows(payload: dict[str, Any], asset_class: str) -> list[dict[str, Any]]:
    ...
    if not isinstance(rows, list):
        raise ValueError("MCX fixture must contain a records list")
    selected = [
        row
        for row in rows
...
def _canonical_record(row: dict[str, Any], fixture_path: Path) -> CanonicalOHLCVRecord:
    ...
    return CanonicalOHLCVRecord(
        ...
        raw_reference=_raw_reference("mcx", row, fixture_path),
...
def _metadata_records(row: dict[str, Any], fixture_path: Path) -> tuple[object, ...]:
    ...
    contract = ContractMetadataRecord(
    ...
    status = PointInTimeMetadataStatusRecord(
        status_id=_stable_id("status", row, fixture_path),
        asset_id=_text(row["asset_id"]),
        source_id=SOURCE_ID,
        metadata_type="contract",
        point_in_time_status=_text(row.get("point_in_time_status", "available")),
        valid_from=_text(row["valid_from"]),
        valid_to=_optional_text(row.get("valid_to")),
        notes="prototype sample fixture",
    )
    return (status, instrument, mapping, contract)


def _duplicate_timestamps(records: tuple[CanonicalOHLCVRecord, ...]) -> int:
...
def _require_fields(row: dict[str, Any], required: tuple[str, ...]) -> None:
    ...
    if missing:
        raise ValueError(f"missing required MCX fields: {', '.join(sorted(missing))}")
